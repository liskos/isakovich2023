...
def clasterisation(data, r):
    ...
    while data:
        ...
        for x1, y1 in clasters[-1]:
            sosedi = [[x2, y2] for x2, y2 in data if max(abs(x1 - x2), abs(y1 - y2)) <= r]
...
def get_centroid(claster):
    ...
    for x1, y1 in claster:
        r += [(sum(max(abs(x1 - x2), abs(y1 - y2)) for x2, y2 in claster), [x1, y1])]
    return min(r)[1]
...
data = [list(map(float, line.split())) for line in open('27data/27-25a.txt')]
...
data = [list(map(float, line.split())) for line in open('27data/27-25b.txt')]
